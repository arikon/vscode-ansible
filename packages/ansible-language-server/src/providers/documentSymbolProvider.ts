...
import { playExclusiveKeywords } from "@src/utils/ansible.js";
import { isTaskKeyword } from "@src/utils/ansible.js";
import { getOrigRange, getYamlMapKeys, parseAllDocuments } from "@src/utils/yaml.js";
...
function processRoles(
  ...
  for (const item of seq.items) {
    if (isScalar(item)) {
      ...
      if (!range) continue;
      const roleName =
        getScalarValue(item, "role") ??
        getScalarValue(item, "name") ??
        "Role";
      const selectionRange =
